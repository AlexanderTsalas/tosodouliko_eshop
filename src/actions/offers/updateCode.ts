...
/** Patches limit / affiliate / active fields on a code. The `code`
 *  string itself is immutable (delete + create to change). */
export async function updateCode(
  ...
  const { data: row, error } = await admin
    ...
    .single();
  if (error || !row) {
    return fail<Code>("Failed to update code: " + error?.message, error?.code);
  }
